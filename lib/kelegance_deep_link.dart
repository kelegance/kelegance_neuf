...
abstract final class KeleganceDeepLink {
  ...
  static const String _prefsKeyConsoleAdmin = 'kelegance_deep_link_console_admin_v1';
  ...
  static const Set<String> _segmentsConsoleAdmin = {
    'console',
  };
  ...
  static bool estRouteGestion(Uri uri) {
    ...
    final role = uri.queryParameters['role']?.toLowerCase();
    if (role == 'driver' || role == 'chauffeur' || role == 'conducteur') {
      return true;
    }
    if (uri.queryParameters['profil']?.toLowerCase() == KeleganceWebUrls.profilHubCollaborateur) {
      return true;
    }
    final segments = uri.pathSegments.where((s) => s.isNotEmpty).toList();
    if (segments.isEmpty) return false;
  ...
  static bool estRouteConsoleAdmin(Uri uri) {
    if (uri.queryParameters['action']?.toLowerCase() == 'console') {
      return true;
    }
    final segments = uri.pathSegments.where((s) => s.isNotEmpty).toList();
    if (segments.isEmpty) return false;
    return _segmentsConsoleAdmin.contains(segments.first.toLowerCase());
  }
  ...
  static Future<void> capturerDepuisUri(Uri? uri) async {
    ...
    if (estRouteConsoleAdmin(uri)) {
      await enregistrerIntentConsoleAdmin();
      if (kDebugMode) {
        debugPrint('Kelegance deep link — intent console admin : ${uri.toString()}');
      }
    }
  }
  ...
  static Future<void> enregistrerIntentConsoleAdmin() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool(_prefsKeyConsoleAdmin, true);
  }
  ...
  static Future<bool> aIntentConsoleAdminEnAttente() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getBool(_prefsKeyConsoleAdmin) ?? false;
  }
  ...
  static Future<bool> consommerIntentConsoleAdmin() async {
    final prefs = await SharedPreferences.getInstance();
    final pending = prefs.getBool(_prefsKeyConsoleAdmin) ?? false;
    if (pending) {
      await prefs.remove(_prefsKeyConsoleAdmin);
    }
    return pending;
  }
